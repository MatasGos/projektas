...
function checkname ($username){   // Vartotojo vardo sintakse
 ...
 function checkdb($username) {  // iesko DB pagal varda, grazina {vardas,slaptazodis,lygis,id} ir nustato name_error
		 ...
		 $sql = "SELECT username, password, userlevel, userid, email, (hour(timestamp)*120+minute(timestamp)*2+Round(second(timestamp)/30,0)) as hours,hour(timestamp) as h,minute(timestamp) as m,minute(timestamp) as m, timestamp FROM " . TBL_USERS. " WHERE username = '$username'";
		 ...
		 if (!$result || (mysqli_num_rows($result) != 1))   // jei >1 tai DB vardas kartojasi, netikrinu, imu pirma
		   ...
		   $ulevel=$row["userlevel"]; $uid= $row["userid"]; $umail = $row["email"];
			$usercred=$row["hours"]; $time = $row["timestamp"]; $s = $row["s"]; $m = $row["m"]; $h = $row["h"];}
     return array($uname,$upass,$ulevel,$uid,$umail, $usercred, $time, $s, $m, $h);
 }
...
function checkmail($mail) {   // e-mail sintax error checking  
   ...
   function updatecreds() {  // iesko DB pagal varda, grazina {vardas,slaptazodis,lygis,id} ir nustato name_error
   $db=mysqli_connect(DB_SERVER, DB_USER, DB_PASS, DB_NAME);
   $id = $_SESSION['userid'];
   $sql = "SELECT (hour(timestamp)*1800+minute(timestamp)*30) as hours, timestamp, hour(timestamp) as h,minute(timestamp) as m,minute(timestamp) as m FROM " . TBL_USERS. " WHERE id = '$id'";
   $result = mysqli_query($db, $sql);
   $uname = $upass = $ulevel = $uid = $umail = null;
   if (!$result || (mysqli_num_rows($result) != 1))   // jei >1 tai DB vardas kartojasi, netikrinu, imu pirma
	 {  // neradom vartotojo DB
	  $_SESSION['name_error']=
	   "<font size=\"2\" color=\"#ff0000\">* Klaida</font>";
   }
else {  //vardas yra DB
	 $row = mysqli_fetch_assoc($result); 
	 $usercred=$row["hours"];
	 ...
	 $m=$row["m"];
	 $h=$row["h"];
	 return array($usercred, $time, $s, $m, $h);
}
   }
 ?>
